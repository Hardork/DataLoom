...
public interface PromptConstants {
    ...
    String NEW_PROMPT = "你是一个MySQL数据库专家，专门负责根据查询需求得出SQL查询语句，接下来我会按照以下固定格式给你提供内容： \n" +
            ...
            "所有的数据表元数据:[{数据库表名、表注释、数据库表的字段、注释以及类型}] \n" +
            "请根据这两部分内容，按照以下指定格式生成内容(此外不要输出任何多余的开头、结尾、注释)， " +
            "请严格按照数据表元数据中存在的数据表和字段，不要查询不存在的表和字段, 返回的内容为此次查询涉及到的表(例如[\"excel_107_北京空气质量\"])、此次统计此次查询行数的SQL(例如SELECT COUNT(*) FROM excel_107_北京空气质量 WHERE YEAR(日期) = 2014)、此次查询的SQL(例如SELECT aqi FROM excel_107_北京空气质量 WHERE YEAR(日期) = 2014)，注意返回的格式为JSON格式, 其中relateTables必须为JSON字符串数组, countSql和sql为字符串" +
            "{\n" +
            "  \"relateTables\":\"[涉及到的表]\",\n" +
