...
public class UserWebSocket {
    ...
    //  注：底下WebSocket是当前类名
    private static CopyOnWriteArraySet<UserWebSocket> webSockets =new CopyOnWriteArraySet<>();
    // 用来存在线连接用户信息
    ...
    public void sendAllMessage(String message) {
        ...
        for(UserWebSocket webSocket : webSockets) {
            try {
    ...
    public void sendOneMessage(long userId, WebSocketMsgVO webSocketMsgVO) {
        ...
        if (session != null && session.isOpen()) {
            try {
                String res = JSONUtil.toJsonStr(webSocketMsgVO);
