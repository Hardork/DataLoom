...
@Component
@Slf4j
public class CouponChainContext<T> implements ApplicationContextAware, CommandLineRunner {

    /**
     * 使用一个Map存储策略组
     * <策略组标识，处理器集合>
     */
    private final Map<String, List<CouponAbstractChainHandler>> abstractChainHandlerContainer = new HashMap<>();
    ...
    public void handle(String mark, T request) {
        List<CouponAbstractChainHandler> chainHandlers = abstractChainHandlerContainer.get(mark);
    ...
    public void run(String... args) throws Exception {
        Map<String, CouponAbstractChainHandler> handlerMap = applicationContext.getBeansOfType(CouponAbstractChainHandler.class);
        handlerMap.forEach((beanName, bean) -> {
            // 获取处理器对应的集合
            List<CouponAbstractChainHandler> list = abstractChainHandlerContainer.getOrDefault(bean.mark(), new ArrayList<>());
            // 将处理器加入对应策略组
            list.add(bean);
            // 更新策略容器
